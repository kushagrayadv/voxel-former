...
def generate_ablation_jobs(base_params, param_ranges, job_params):
    ...
    for values in itertools.product(*param_values):
        ...
        model_name = f"{params['base']['wandb_project']}_{'_'.join([f'{name}_{value}' for name, value in zip(param_names, values)])}"
        params['base']['model_name'] = model_name + f"_{datetime.now().strftime('%Y%m%d')}"
        job_name = model_name
...
default_params = {
    "base": {
        ...
        "model_name": "main_no_prior_linformer",
    },
    "data": {
