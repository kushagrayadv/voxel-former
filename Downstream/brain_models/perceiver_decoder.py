    ...
    def forward(self, x, coords):
        ...
        for b in range(batch_size):
            ...
            if self.method == 'grid':
                ...
                cells_per_dim = max(2, int(target_cells**(1/3)))  # Cube root for 3D, minimum 2
                
                # 3. Compute grid indices using vectorized operations
                normalized_coords = (batch_coords - min_coords) / coord_range
                grid_indices = (normalized_coords * cells_per_dim).long().clamp(0, cells_per_dim-1)
                
                # 4. Compute flattened indices more efficiently
                # Use stride multipliers for each dimension
                strides = torch.tensor([cells_per_dim * cells_per_dim, cells_per_dim, 1], 
                                     device=device)
                flat_indices = (grid_indices * strides).sum(dim=1)
                
                # 5. Find unique cells and inverse mapping - this is still needed
                unique_cells, inverse, counts = torch.unique(flat_indices, return_inverse=True, return_counts=True)
                ...
                indices = torch.stack([inverse, torch.arange(num_tokens, device=device)])
        ...
        padded_coords = torch.zeros((batch_size, max_len, coords.shape[-1]), device=device)
        masks = torch.zeros((batch_size, max_len, 1), device=device)
        
        # Use vectorized operations for padding
        for b, (b_x, b_coords, size) in enumerate(zip(result_x, result_coords, all_downsample_sizes)):
            padded_x[b, :size] = b_x
...
class HierarchicalPerceiverDecoder(nn.Module):
    def __init__(
        ...
        self.pos_embed = SirenPositionalEmbedding(in_features=coord_dim, out_features=h, omega_0=omega_0)
        ...
        if use_residual:
            ...
            for i in range(n_blocks - 1):
                ...
                merge_attn = PreNorm(h, Attention(
                    query_dim=h,
                    context_dim=h,
                    heads=num_heads,
                    dim_head=head_dim,
    ...
    def forward(self, x, coords=None):
        ...
        for i, downsample in enumerate(self.downsample_layers):
            down_x, down_coords, down_mask = downsample(contexts[-1], coord_hierarchy[-1])
